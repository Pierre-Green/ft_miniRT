...
/*   Updated: 2020/05/11 16:02:43 by pguthaus         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "world.h"

static t_bool		cylinder_intersects_2(t_vec3f hty, t_vec3f abc,
	t_vec3f cardoc, float *dist)
{
	hty.x = sqrt(hty.x);
	...
	t_vec3f dir, float *dist)
{
	...
	ca = v3f_sub(v3f_add(cylinder.position,
		v3f_mul(cylinder.orientation, cylinder.height)), cylinder.position);
	...
	if (hty.x < 0)
		return (false);
	return (cylinder_intersects_2(hty, abc, cardoc, dist));
}
	...
	t_second_ray next_ray)
{
	...
	top_center = v3f_add(cylinder.position,
		v3f_mul(cylinder.orientation, cylinder.height));
	...
	else
	{
		...
		next_ray.normal = v3f_normalize(v3f_sub(next_ray.origin,
			v3f_add(cylinder.position, v3f_mul(cylinder.orientation, tmp2))));
	}
	...
	t_second_ray *next_ray, float *dist)
{
	...
	does_intersect = cylinder_intersects(cylinder, next_ray->origin,
