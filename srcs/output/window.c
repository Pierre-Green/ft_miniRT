...
/*   Updated: 2020/05/11 15:44:45 by pguthaus         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				move(int key, t_carry *c)
{
	...
	if (key == KEY_W)
		cam->position = v3f_add(cam->position, v3f_mul(cam->forward, V_MOVE));
	else if (key == KEY_S)
		cam->position = v3f_add(cam->position, v3f_mul(cam->forward,
			-1 * V_MOVE));
	else if (key == KEY_A)
		cam->position = v3f_add(cam->position, v3f_mul(cam->right,
			-1 * V_MOVE));
	else if (key == KEY_D)
		cam->position = v3f_add(cam->position, v3f_mul(cam->right, V_MOVE));
	else if (key == KEY_R)
		cam->position = v3f_add(cam->position, v3f_init(0, V_MOVE, 0));
	else if (key == KEY_F)
		cam->position = v3f_add(cam->position, v3f_init(0, -1 * V_MOVE, 0));
}
...
static int				on_key_pressed(int key, void *p_c)
{
	...
	if (key == KEY_W || key == KEY_A || key == KEY_S || key == KEY_D
		...
		|| key == KEY_ARROW_RIGHT || key == KEY_ARROW_UP)
...
void					init_window(t_carry *c)
{
	...
	if (!(c->s->win_ptr = mlx_new_window(c->s->mlx_ptr, c->w->res[0],
		c->w->res[1], "MiniRT")))
		freexit(21, "Failed to mlx_new_window()", c);
	if (!(c->s->img_ptr = mlx_new_image(c->s->mlx_ptr, c->w->res[0],
		c->w->res[1])))
		freexit(22, "Failed to mlx_new_image()", c);
	c->s->image = mlx_get_data_addr(c->s->img_ptr, &(c->s->bits_per_pixel),
		&(c->s->size_line), &(c->s->endian));
